...
router.get('/me', requireTelegramAuth, async (req, res) => {
  ...
  // Count how many people this user has successfully referred (paid
  // bonuses only, i.e. the referred user completed the required join)
  ...
  return res.json({
    user: { ...user, referral_count: referralCount ?? 0, referral_link: referralLink, is_admin: isAdmin }
  });
});
