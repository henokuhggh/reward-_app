 ...
 */
router.get('/payment-methods', requireAnyAuth, async (req, res) => {
 ...
 */
router.post('/deposits', requireAnyAuth, async (req, res) => {
 ...
 */
router.get('/deposits/mine', requireAnyAuth, async (req, res) => {
  const user = req.user;
 ...
 */
router.get('/deposit-terms', requireAnyAuth, async (req, res) => {
  const { data: settings, error } = await supabase.from('app_settings').select('min_deposit').single();
  if (error) return res.status(500).json({ error: error.message });

  return res.json({
    minDeposit: settings?.min_deposit ?? 1000
  });
});
 ...
 */
router.get('/withdrawal-terms', requireAnyAuth, async (req, res) => {
 ...
 */
router.post('/withdrawals', requireAnyAuth, async (req, res) => {
 ...
 */
router.get('/withdrawals/mine', requireAnyAuth, async (req, res) => {
  const user = req.user;
  if (!user) return res.status(404).json({ error: 'User not found' });
