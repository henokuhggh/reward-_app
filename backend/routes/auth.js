...
router.get('/me', require('../lib/webAuth').requireAnyAuth, async (req, res) => {
  ...
  const referralLink = siteUrl ? `${siteUrl.replace(/\/$/, '')}/register.php?ref=${req.user.referral_code}` : null;

  // A phone-based user counts as an admin if their phone number
  // matches a row in `admins` - mirrors the Telegram-side check in
  // GET /api/users/me. Grants no actual access by itself; every
  // /api/admin/* route independently re-checks via requireAdmin.
