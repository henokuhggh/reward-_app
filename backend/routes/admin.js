...
/**
 * GET /api/admin/bingo/stats
 * Platform earnings from the 20% cut, rounds played/live, and
 * total birr wagered - the headline bingo card on the dashboard.
 */
router.get('/bingo/stats', async (req, res) => {
  const { data, error } = await supabase.rpc('admin_bingo_stats');
  if (error) return res.status(500).json({ error: error.message });

  const stats = data?.[0] || {};
  return res.json({
    totalPlatformEarnings: stats.total_platform_earnings ?? 0,
    totalRoundsFinished: stats.total_rounds_finished ?? 0,
    totalRoundsLive: stats.total_rounds_live ?? 0,
    totalWagered: stats.total_wagered ?? 0
  });
});

/**
 * GET /api/admin/bingo/stakes
 * All stake tiers including inactive ones, for the admin management view.
 */
router.get('/bingo/stakes', async (req, res) => {
...
/**
 * POST /api/admin/bingo/stakes
 * Body: { amount, sortOrder? }
 */
router.post('/bingo/stakes', async (req, res) => {
  const { amount, sortOrder } = req.body || {};
  ...
  const { data: stake, error } = await supabase
    .from('bingo_stake_tiers')
    .insert({ amount, sort_order: sortOrder || 0 })
    .select('*')
    .single();
...
/**
 * PATCH /api/admin/bingo/stakes/:id
 * Body: any subset of { amount, sortOrder, isActive }
 */
router.patch('/bingo/stakes/:id', async (req, res) => {
  const { amount, sortOrder, isActive } = req.body || {};
...
/**
 * GET /api/admin/bingo/rounds?status=finished
 * Recent rounds for the admin history view, newest first.
 */
router.get('/bingo/rounds', async (req, res) => {
  const status = req.query.status;

  let query = supabase
    .from('bingo_rounds')
    .select('id, status, stake_amount, total_pool, platform_cut, prize_pool, winner_ids, win_pattern, started_at, finished_at, created_at')
    .order('created_at', { ascending: false })
    .limit(100);
...
/**
 * POST /api/admin/bingo/rounds/:id/cancel
 * Aborts a waiting/active round and refunds every entrant. Used if
 * a round is stuck (e.g. only 1 player joined and they want out).
 */
router.post('/bingo/rounds/:id/cancel', async (req, res) => {
  const { error } = await supabase.rpc('cancel_bingo_round', { p_round_id: req.params.id });
