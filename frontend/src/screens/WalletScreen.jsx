...
function DepositFlow({ onBalanceChange }) {
  ...
  useEffect(() => {
    api
      .getDepositTerms()
      .then(({ minDeposit }) => setMinDeposit(minDeposit))
      .catch(() => {});
  }, []);
  ...
  async function goToStep2() {
    ...
    if (!numAmount || numAmount < minDeposit) {
      setError(`Minimum deposit is ${minDeposit} birr`);
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="flow">
      ...
      {step === 1 && (
        <div className="step-card">
          ...
          <p className="hint-text">Minimum deposit is {minDeposit.toLocaleString()} birr. No maximum.</p>
...
function WithdrawFlow({ user, onBalanceChange }) {
  ...
  const [submitted, setSubmitted] = useState(false);
  const [history, setHistory] = useState([]);
  const [terms, setTerms] = useState({ minWithdrawal: 200, feePercent: 10 });
  ...
  useEffect(() => {
    api.getWithdrawalTerms().then(setTerms).catch(() => {});
  }, []);

  const numAmount = Number(amount) || 0;
  const feeAmount = Math.round(numAmount * (terms.feePercent / 100) * 100) / 100;
  const netAmount = Math.round((numAmount - feeAmount) * 100) / 100;

  async function submit() {
    setError('');
    if (!numAmount || numAmount < terms.minWithdrawal) {
      setError(`Minimum withdrawal is ${terms.minWithdrawal} birr`);
      return;
    }
  ...
  return (
    <div className="flow">
      ...
      <div className="step-card">
        ...
        <p className="hint-text">
          Minimum withdrawal is {terms.minWithdrawal} birr. A {terms.feePercent}% transaction fee applies.
        </p>
        ...
        {numAmount > 0 && (
          <div className="fee-breakdown">
            <div className="fee-row">
              ...
              <span>{numAmount.toLocaleString()} ETB</span>
            </div>
            <div className="fee-row fee-row-deduct">
              <span>Transaction fee ({terms.feePercent}%)</span>
              <span>-{feeAmount.toLocaleString()} ETB</span>
            </div>
        ...
        <div className="history-section">
          ...
          {history.map((w) => (
            <HistoryRow
              ...
              status={w.status}
              date={w.requested_at}
              subtitle={`${w.method} - ${Number(w.net_amount ?? w.amount).toLocaleString()} ETB after fee`}
            />
          ))}
        </div>
      )}
