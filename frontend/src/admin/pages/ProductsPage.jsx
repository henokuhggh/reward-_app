...
function ProductForm({ onChange }) {
  ...
  async function handleFileChange(e) {
    ...
    setUploading(true);
    try {
      const { imageUrl } = await api.admin.uploadProductImage(file);
  ...
  return (
    <form className="product-form" onSubmit={handleSubmit}>
      ...
      <div className="image-upload-row">
        {form.imageUrl && <img src={form.imageUrl} alt="" className="image-preview" />}
        <label className="file-btn">
          {uploading ? 'Uploading...' : form.imageUrl ? 'Change photo' : 'Upload photo'}
          <input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={handleFileChange} disabled={uploading} hidden />
        </label>
      </div>
      <input
