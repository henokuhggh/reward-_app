 ...
 */
router.get('/links', requireTelegramAuth, async (req, res) => {
 ...
 */
router.get('/bonus', requireTelegramAuth, async (req, res) => {
 ...
 */
router.post('/bonus/:id/claim', requireTelegramAuth, async (req, res) => {
  const user = await getUser(req.telegramUser.id);
  if (!user) return res.status(404).json({ error: 'User not found, call /session first' });
