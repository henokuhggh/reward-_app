...
router.post('/session', requireTelegramAuth, async (req, res) => {
  ...
  // Referrer gets their 10 birr as soon as the referred user
  // registers (not on deposit) - fire-and-forget-safe since
  // credit_referral_bonus is idempotent via a unique constraint.
  if (newUser?.referred_by) {
    const { error: refErr } = await supabase.rpc('credit_referral_bonus', {
      p_referrer_id: newUser.referred_by,
      p_referred_user_id: newUser.id
    });
    if (refErr) console.error('credit_referral_bonus failed:', refErr.message);
  }
...
router.get('/me', requireTelegramAuth, async (req, res) => {
  ...
  // independently re-checks ADMIN_TELEGRAM_IDS via requireAdmin, so
  // a modified client claiming isAdmin=true still gets rejected
  // server-side on any real admin action.
  const adminIds = (process.env.ADMIN_TELEGRAM_IDS || '').split(',').map((id) => id.trim());
  const isAdmin = adminIds.includes(String(req.telegramUser.id));
