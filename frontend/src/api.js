...
export const api = {
  ...
  admin: {
    ...
    banUser: (phone, banned) =>
      request('/api/admin/users/ban', {
        method: 'POST',
        body: JSON.stringify({ phone, banned })
      }),

    removeUserByPhone: (phone) =>
      request(`/api/admin/users/by-phone/${encodeURIComponent(phone)}`, { method: 'DELETE' }),

    setUserEarningPaused: (userId, paused) =>
      request(`/api/admin/users/${userId}/earning`, {
        method: 'POST',
        body: JSON.stringify({ paused })
      }),

    setGlobalEarningPaused: (paused) =>
      request('/api/admin/earning/global', {
        method: 'POST',
        body: JSON.stringify({ paused })
      }),

    resetUserPurchases: (userId) =>
      request(`/api/admin/users/${userId}/reset-purchases`, { method: 'POST' }),

    giftProduct: (phone, productId) =>
      request('/api/admin/users/gift-product', {
        method: 'POST',
        body: JSON.stringify({ phone, productId })
      }),

    adjustUserBonus: (phone, delta) =>
      request('/api/admin/users/adjust-bonus', {
        method: 'POST',
    ...
    addAdmin: ({ telegramId, telegramUsername, phone, role }) =>
      request('/api/admin/admins', {
        method: 'POST',
        body: JSON.stringify({ telegramId, telegramUsername, phone, role })
      }),
