...
router.get('/me', async (req, res) => {
  return res.json({ role: req.admin.role, telegramId: req.admin.telegram_id });
});
 ...
 * Body: { telegramId, telegramUsername?, role }
 * Owner-only: adds a new admin (owner or sub_admin).
 */
router.post('/admins', requireOwner, async (req, res) => {
  const { telegramId, telegramUsername, role } = req.body || {};

  if (!telegramId || !['owner', 'sub_admin'].includes(role)) {
    return res.status(400).json({ error: 'telegramId and a valid role (owner or sub_admin) are required' });
  }
