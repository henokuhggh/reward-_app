...
export default function ReferralCard({ user }) {
  ...
  function handleShare() {
    if (!link) return;
    const shareText = `Join and start earning ETB - use my link:`;
  ...
  return (
    <div className="referral-card">
      <div className="referral-header">
        ...
        <span className="referral-count">{user?.referral_count ?? 0} joined</span>
      </div>
      <p className="note-body">
        Share your link below. You earn a bonus once someone you invite joins our
        required channel.
      </p>
