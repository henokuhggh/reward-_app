...
function AddAdminForm({ onChange }) {
  const [method, setMethod] = useState('telegram'); // 'telegram' | 'phone'
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (method === 'telegram') {
        ...
        await api.admin.addAdmin({ phone, role });
      }
      setTelegramId('');
      setTelegramUsername('');
      setPhone('');
  ...
  return (
    <form className="admin-form" onSubmit={handleSubmit}>
      ...
      <div className="method-toggle">
        <button type="button" className={method === 'telegram' ? 'active' : ''} onClick={() => setMethod('telegram')}>
          By Telegram ID
        </button>
      ...
      <p className="card-hint">
        ...
        {method === 'phone' && ' A phone-based admin logs in through the normal website phone/password login.'}
      </p>
  ...
  return (
    <div className="admin-row">
      <div className="admin-row-info">
        <span className="admin-row-title">{admin.telegram_username || admin.phone || admin.telegram_id}</span>
        <span className="admin-row-meta">{admin.phone ? admin.phone : admin.telegram_id}</span>
      </div>
